...
extension LayoutArranging {
    ...
    func arrange(task: ArrangingTask) -> LayoutArrangement {
        ...
        arrangedItems += arrangeFullyFrozenItems(
                &items,
                flow: flow,
                occupiedIndices: &occupiedIndices,
                growingTracksCount: &growingTracksCount
        )
        arrangedItems += arrangeSemiFrozenItems(
                &items,
                flow: flow,
        ...
        arrangedItems += arrangeNonFrozenItem(
                items,
                flow: flow,
                fixedTracksCount: fixedTracksCount,
                packing: packing,
    ...
    private func arrangeFullyFrozenItems(_ items: inout [ArrangementInfo], flow: GridFlow, occupiedIndices: inout [GridIndex],
        ...
        staticItems.forEach { staticItem in
            let itemIndex = items.firstIndex { $0.gridElement == staticItem.item }!
            guard !occupiedIndices.contains(staticItem.start, span: staticItem.span) else {
                    ...
                    items[itemIndex...itemIndex] = [
                        .init(gridElement: prevItem.gridElement,
                              start: GridStart.default,
                              span: prevItem.span)
                    ]
                    return
                }
    ...
    private func arrangeSemiFrozenItems(
            _ items: inout [ArrangementInfo],
            flow: GridFlow,
            fixedTracksCount: Int,
            occupiedIndices: inout [GridIndex],
            growingTracksCount: inout Int
    ) -> [ArrangedItem] {
        var result: [ArrangedItem] = []
        // Arrange static items with frozen start in fixed flow dimension
        for dimension in GridFlow.Dimension.allCases {
    ...
    private func arrangeNonFrozenItem(
            _ items: [ArrangementInfo],
            flow: GridFlow,
            fixedTracksCount: Int,
            packing: GridPacking,
            occupiedIndices: inout [GridIndex],
...
fileprivate extension GridIndex {
    func nextIndex(fixedTracksCount: Int, flow: GridFlow, span: GridSpan, frozenDimension: GridFlow.Dimension? = nil) -> GridIndex? {
