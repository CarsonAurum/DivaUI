//
//  Created by Carson Rau on 8/3/21.
//
...
struct PositionedLayout: Equatable {
  ...
  subscript(gridElement: GridElement) -> PositionedItem? {
    items.first { $0.gridElement == gridElement }
  }

  subscript(arrangedItem: ArrangedItem) -> PositionedItem? {
    items.first { $0.gridElement == arrangedItem.gridElement }
  }
}
...
extension LayoutPositioning {
  func reposition(_ task: PositioningTask) -> PositionedLayout {
    ...
    let growingTracksSizes: [CGFloat] = calculateTrackSizes(
            task: task,
    ...
    let fixedTracksSizes = calculateTrackSizes(
            task: task,
  ...
  private func calculateTrackSizes(task: PositioningTask, boundingSize: CGFloat,
                                   tracks: [GridTrack], dimension: GridFlow.Dimension) -> [CGFloat] {
    let flow = task.flow
    let arrangement = task.arrangement
    ...
    sizeToFitNonSpanning(growingTracksSizes: &growingTracksSizes, arrangement: arrangement,
    ...
    expandFlexibleTracks(&growingTracksSizes, boundingSize)
  ...
  private func positionedItems(task: PositioningTask, growingTracksSizes: [CGFloat], fixedTracksSizes: [CGFloat]) -> PositionedLayout {
    ...
    for positionedItem in task.items {
      ...
      case .scroll:
        itemGrowingSize =
          (arrangedItem.startIndex[keyPath: flow.index(.growing)]...arrangedItem.endIndex[keyPath: flow.index(.growing)])
                  .reduce(0) { $0 + growingTracksSizes[$1] }
        let centringCorrection = (itemGrowingSize - positionedItem.bounds.size[keyPath: flow.size(.growing)]) / 2
        growingPosition = (0..<arrangedItem.startIndex[keyPath: flow.index(.growing)])
                .reduce(0) {$0 + growingTracksSizes[$1]} + centringCorrection
      }

      let fixedTrackStart = fixedTracksSizes[0..<arrangedItem.startIndex[keyPath: flow.index(.fixed)]]
              .reduce(0, +)
      let fixedTrackSize =
              fixedTracksSizes[arrangedItem.startIndex[keyPath: flow.index(.fixed)]...arrangedItem.endIndex[keyPath: flow.index(.fixed)]]
                      .reduce(0, +)
  ...
  private func sizeToFitNonSpanning(
          growingTracksSizes: inout [PositionedTrack],
          arrangement: LayoutArrangement,
          flow: GridFlow,
          dimension: GridFlow.Dimension,
